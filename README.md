...
Есть **два способа** — оба с одинаковым красивым полноэкранным TUI (прогресс-бар,
детали по `^O`, финальная сводка). Разница только в том, спрашивает ли установщик
конфигурацию перед стартом.

### 🅰️ Основная команда — с интерактивным меню выбора (рекомендуется)

Запуск **без флагов** — установщик сам спросит режим доступа к панели и набор
компонентов через интерактивное меню, затем покажет полноэкранный TUI установки:

```bash
curl -fsSL https://raw.githubusercontent.com/krygag1234-a11y/Olc-cost-l/main/install.sh | sudo bash
```

> Если система уже установлена — вместо меню компонентов появится меню действий
> (Доустановка / Обновление / Переустановить с нуля / Отмена).
...
> Хотите меню выбора компонентов даже с флагами? Добавьте `--interactive`.

<details>
<summary>Полный список флагов установки</summary>

| Флаг | Назначение |
|---|---|
| (без флагов) | интерактивное меню выбора компонентов + TUI |
| `--full` | полная установка без вопросов (Tor+мосты+Split+Zapret+Панель) |
| `--interactive` | показать меню выбора даже вместе с флагами |
| `--tor` / `--with-tor` | Tor + Панель |
| `--split` | Split-routing + Панель (требует Tor) |
| `--zapret` | Zapret (DPI-bypass) + Панель |
| `--bridges` | мосты Tor + Панель (требует Tor) |
| `--warp` / `--with-warp` | Cloudflare WARP + Панель (зарубежный VPS, без Tor) |
...
Конфликтующие комбинации (`--warp` вместе с `--tor`, `--split`/`--bridges` при
выключенном Tor) установщик отклонит с понятной ошибкой.
...
Для первого перехода со старого pipeline используется проверенный отдельный
checkout через `OLC_REINSTALL_SOURCE_DIR`; старый `/opt` при этом остаётся внутри
полного rollback-архива. Подробная команда приведена в `docs/UPDATE.md`.
