...
- **WARP** в toggle-блоках показывается только когда реально установлен на VPS (например, есть `warp-cli`); установка/удаление — через drawer **«Компоненты VPS»** (±) с job-логом (`GET /api/components/jobs`).
