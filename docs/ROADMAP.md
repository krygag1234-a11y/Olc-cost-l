...
> **Текущая версия панели (репо):** `main` @ `39f4c61` (UI v3: toggle + логи; настройки — заглушки)
...
| ID | Задача | Статус |
|----|--------|--------|
| 1.2.1 | `install.sh` — после разбора argv писать профиль (`--no-tor`, `--no-zapret`, `--no-split`, …) | `[ ]` |
| 1.2.2 | `agent-bootstrap.sh` — при `--update` читать профиль, пропускать чужие шаги | `[ ]` |
| 1.2.3 | `olc-update.sh` — опции `--profile`, `--set-profile`, `--show-profile` | `[ ]` |
| 1.2.4 | Симлинк/хелпер `olc-profile` — смена профиля без переустановки | `[ ]` |
...
| ID | Задача | Статус |
|----|--------|--------|
| 2.1 | Go: `capabilitiesHandler` — читает `features.env`, systemd, файлы, `install-state` | `[ ]` |
| 2.2 | UI: `useCapabilities()` — скрывает блоки Zp/Tor/Sp/Мосты | `[ ]` |
| 2.3 | При `split` без `tor` — split скрыт или disabled с tooltip | `[ ]` |
...
| ID | Настройка | Статус |
|----|-----------|--------|
| 3.4.1 | Переименование в UI (Wt → **Мосты** или **Br**) | `[ ]` |
