...
Устройство может иметь `label` во всех четырёх областях интерфейса: глобальные
🎫/🔌 и per-client 🎫/🔌. Имя сохраняется при переносе между allowlist и ban,
редактируется и в забаненной строке, используется журналами вместо голого HWID
(сам HWID остаётся рядом) и входит в backup вместе с `access-control.json`.
...
## Автосмена оригинальных ключей

Перед каждым кругом manager пропускает инстанс, если у него есть активные peers
или за последние две минуты зарегистрированы попытки подключения. Постоянные
переподключения поэтому считаются активностью так же, как живой туннель. Такой
инстанс ждёт следующего штатного круга через N часов; внеочередная ротация не
запускается.
...
`HMAC-SHA256(randomization_secret, client_id || 0x00 || room_id)`.

Seed привязан к идентичности инстанса и не зависит от `Endpoint.Key`. Поэтому
штатная автосмена оригинального ключа не меняет рандомизированный ключ типа 1.
...
`OLCRTC_ALT_KEY_MODE=2`, `OLCRTC_ALT_KEY_SECRET` и стабильный
`OLCRTC_ALT_KEY_SEED`; core выводит ключ для текущей
...
Подписи HWID хранятся в `access-control.json.device_labels`, показываются во
всех глобальных/выборочных журналах 🎫 и 🔌 и переживают переносы между
разрешёнными и забаненными списками. Неиспользуемая подпись удаляется через 30
дней, только если HWID отсутствует во всех четырёх типах списков и столько же не
появлялся ни в одном журнале.

При переводе устройства в разрешённые его старые счётчики отказов/сбросов
очищаются и карточка показывает синий статус «Разрешённый». После удаления из
разрешённых новые счётчики начинаются с нуля.
