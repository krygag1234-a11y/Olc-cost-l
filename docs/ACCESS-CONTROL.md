...
Актуально для production-сборки Olc-cost-l от 2026-08-01. Документ описывает
...
- `monitor` / «выключено» — allowlist выключен; при активной рандомизации
  client_id оригинальный URL не даёт обхода, а рандомизированный остаётся
  доступен всем, кроме ban-листа;
- `keyrand` / «+» — разрешённые устройства могут использовать оригинальный и
  рандомизированный client_id, неизвестные — только рандомизированный;
- `enforce` / «Блокировать неизвестных» — пропускать только активные записи
  allowlist по оригинальному или рандомизированному client_id; ban имеет
  приоритет.
...
|---|---|---|---|---|
| `off` | запрещён при активной crypto-рандомизации; без неё обычный ключ разрешён | разрешён | выключен и полностью игнорируется | всегда блокирует |
...
Точная принятая матрица:

- `off`: разрешённое и неизвестное устройство + оригинальный ключ → reject;
- `off`: разрешённое и неизвестное устройство + рандомизированный ключ → allow;
- `keyrand`: разрешённое устройство + оригинальный ключ → allow;
- `keyrand`: неизвестное устройство + оригинальный ключ → reject;
- `keyrand`: неизвестное устройство + рандомизированный ключ → allow;
- `enforce`: только разрешённое устройство получает доступ по любому из двух
  валидных классов ключа;
- забаненное устройство с любым ключом → reject.

Класс ключа фиксируется transport-слоем на первом успешно расшифрованном фрейме и
сохраняется у живой peer-сессии. Поэтому handshake и повторная проверка watcher
используют одну матрицу: переключение из `+` в `off` либо выключение записи
allowlist немедленно сбрасывает сессию, которая держалась на оригинальном ключе.
...
остаётся открытым. Пустой allowlist в `enforce` блокирует всех. При ошибке
чтения/разбора access JSON списки работают fail-open, но crypto-рандомизация
остаётся обязательной: keyClass=0 не получает обход из-за отсутствующего файла.
...
- `TestOlcAccessKeyrandMatrix` и `TestOlcAccessLiveKeyClassMatrix` —
  `off/keyrand/enforce`, отсутствие access-конфига, type 1/type 2, ban,
  отключённая запись allowlist и selective room;
