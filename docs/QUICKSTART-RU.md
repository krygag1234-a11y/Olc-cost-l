...
После `--full` по умолчанию: `https://IP_ВАШЕГО_VPS:8888/admin` (self-signed, браузер покажет предупреждение).

Для HTTPS без предупреждения на публичном IP: `sudo olc-update --ip --https-letsencrypt`. HTTP используется только после явного `--http`.
...
3. Для Jitsi заполните **Jitsi Server** и **Room ID**. Полную meet-ссылку можно вставить в любое поле — панель разделит её автоматически.
4. Если известен только публичный HTTP/IP Jitsi, нажмите **«Найти HTTPS-домен»**, проверьте найденный `https://domain:443` и нажмите **«Использовать»**. Доверенный TLS и совместимый Jitsi с просроченным/недоверенным сертификатом показываются раздельно; второй вариант требует `OLCRTC_JITSI_INSECURE_TLS=1`. Комната при этом не меняется.
5. Для Telemost/WB Stream укажите только ID комнаты.
6. **QR** — ссылка для Olcbox на телефоне/ПК
