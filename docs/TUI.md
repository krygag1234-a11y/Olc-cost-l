...
- **С флагами** (`--full`, `--tor`, `--warp`, `--no-zapret`, `--ssh`, …) меню
  пропускается ПОКОМПОНЕНТНО: каждый флаг гасит только тот выбор, который он
  задаёт. Например, `--full` задаёт набор компонентов, но не режим доступа —
  поэтому всплывёт короткое меню IP/SSH. Полностью без вопросов = задать флагами
  все измерения (`--full --ip` или `--full --ssh`). Полноэкранный TUI прогресса
  (шапка, бар N/13, `^O`, финальная анимация, сводки) есть в любом случае.
  Конфликтующие флаги отклоняются `tui_fatal`.
