...
This command is for systems already installed by the vendored pipeline. The
first migration of a legacy production VPS must use the dedicated migration plan.
EOF
}
...
[[ -x "$INSTALL_DIR/install.sh" ]] || die "vendored installer not found: $INSTALL_DIR/install.sh"
[[ -x "$INSTALL_DIR/scripts/olc-purge.sh" ]] || die "purge script not found"
[[ -x "$INSTALL_DIR/scripts/olc-backup.sh" ]] || die "logical backup script not found"
[[ -x "$INSTALL_DIR/scripts/olc-vps-backup.sh" ]] || die "VPS backup script not found"
...
cp -a "$INSTALL_DIR/scripts/olc-purge.sh" "$WORK_DIR/olc-purge.sh"
cp -a "$INSTALL_DIR/scripts/olc-vps-backup.sh" "$WORK_DIR/olc-vps-backup.sh"
cp -a "$INSTALL_DIR/scripts/lib-vps-backup.sh" "$WORK_DIR/lib-vps-backup.sh"

PHASE="source-snapshot"
SOURCE_ARCHIVE="$WORK_DIR/vendored-source.tar.gz"
# node_modules is generated by npm ci. Everything else, including .git and any
# intentional local source change, is preserved so reinstall never depends on
# GitHub availability and recreates exactly the source that initiated it.
tar -C "$INSTALL_DIR" \
...
bash "$INSTALL_DIR/scripts/olc-backup.sh" export "$LOGICAL_BACKUP"
...
OLC_VPS_BACKUP_FORCE=1 OLC_VPS_BACKUP_ONCE_PER_DAY=0 bash "$INSTALL_DIR/scripts/olc-vps-backup.sh" create pre-reinstall
