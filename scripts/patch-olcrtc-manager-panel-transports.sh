...
if new_block in t:
    print("[patch-panel-transports] already olcbox-aligned"); raise SystemExit(0)
else:
    t = re.sub(
    ...
    if new_block not in t:
        print("patch-panel-transports: failed to replace block"); raise SystemExit(0)
panel.write_text(t)
...
print("[patch-panel-transports] ok (olcbox: DC/VP8/SEI; no video in panel)"); raise SystemExit(0)
PY
