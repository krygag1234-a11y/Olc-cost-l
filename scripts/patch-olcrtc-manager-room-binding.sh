...
if "Room:   olcrtcRoomConfig{ID: loc.Endpoint.RoomID}," in t:
    print("[patch-room-binding] already bare room id")
    raise SystemExit(0)

if "bindingRoomURL(loc.Carrier" not in t:
    print("[patch-room-binding] skip: bindingRoomURL not in serverConfig")
    raise SystemExit(0)
...
p.write_text(t)
print("[patch-room-binding] ok (reverted telemost URL prefix in server yaml)")
PY
