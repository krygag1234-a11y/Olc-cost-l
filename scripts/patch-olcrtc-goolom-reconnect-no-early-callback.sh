#!/usr/bin/env bash
# upstream master already removed the early onReconnect(nil) call in lifecycle.go.
...
p = Path(sys.argv[1])
t = p.read_text()
# upstream master already removed the early callback; marker check is for old pins.
...
# On upstream master the block simply no longer exists — nothing to do.
if "s.onReconnect(nil)" not in t:
    print("[patch-goolom-reconnect-no-early-callback] ok (upstream master already removed it)"); raise SystemExit(0)
    raise SystemExit(0)
