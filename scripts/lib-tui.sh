...
tui_menu() {
  ...
  # Стереть кадр меню целиком: после выбора меню НЕ должно оставаться на
  # экране. Боевой баг: меню рисуется в ОСНОВНОМ буфере, TUI дальше уходит в
  # alternate screen, а при выходе из него основной буфер восстанавливается —
  # и «мёртвое» меню продолжало висеть сверху над финальной сводкой.
  if [[ "$drawn" == "1" ]]; then
    printf '\033[%dA\033[J' "$rows" >"$tty" 2>/dev/null || true
  fi
