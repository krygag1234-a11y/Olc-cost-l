...
if fp_start < 0:
    print("[patch-panel-hotfix-v7] skip: no FeaturesPanel", file=sys.stderr)
    sys.exit(0)
fp_end = t.find("\nfunction ", fp_start + 1)
if fp_end < 0:
    fp_end = len(t)
...
if net_idx >= 0 and '{collapsed ? "Развернуть" : "Свернуть"}' not in fp[net_idx : net_idx + 1200]:
    if hdr_old in fp:
        ...
        print("[patch-panel-hotfix-v7] warn: FeaturesPanel header pattern not found", file=sys.stderr)
...
p.write_text(t)
print("[patch-panel-hotfix-v7] ok")
PY
