#!/usr/bin/env bash
# Phase 2C Step 4: Keep "Применить изменения" compact and visually consistent
...
# Idempotency check: search for the final compact button marker.
if grep -q "split-apply-compact" "$TARGET" 2>/dev/null; then
...
# New structure: a normal section and an outlined button, matching "Анализировать".
new_section = '''                <section data-ui="split-apply-compact" className="rounded-md border border-border bg-muted/20 p-3 space-y-2">
...
print("[split-2c-step4] compact Apply section: ok", file=sys.stderr)
