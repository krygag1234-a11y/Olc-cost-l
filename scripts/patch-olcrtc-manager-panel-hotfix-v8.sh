...
if "function componentJobUiVisible" in t:
    print("[patch-panel-hotfix-v8] componentJobUiVisible already defined")
    sys.exit(0)

if "componentJobUiVisible" not in t:
    print("[patch-panel-hotfix-v8] skip: no usages")
    sys.exit(0)
...
if anchor not in t:
    print("[patch-panel-hotfix-v8] failed: no COMPONENT_DRAWER_ITEMS anchor", file=sys.stderr)
    sys.exit(1)
...
p.write_text(t)
print("[patch-panel-hotfix-v8] ok")
PY
