...
#   7. после выбора кадр меню СТИРАЕТСЯ (ESC[NA + ESC[J) — боевой баг:
#      меню «висело» сверху в основном буфере после выхода из alt-screen.
...
# Меню из 3 опций: кадр = 3+3=6 рядов; после выбора он должен стереться
# (курсор вверх на 6 + очистка до конца экрана) — иначе меню «висит» в
# основном буфере и всплывает после выхода TUI из alternate screen.
check("кадр меню стирается после выбора (ESC[6A + ESC[J)",
      "\x1b[6A\x1b[J" in s1)
...
check("кадр меню стирается и при выборе стрелками", "\x1b[6A\x1b[J" in s2)
...
printf '[tui-menu-test] OK: меню видно, таймаут не роняет, цифры/стрелки/Enter, чистый stdout, кадр стирается\n'
