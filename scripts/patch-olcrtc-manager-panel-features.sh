...
if "FeaturesPanel" in src:
    print("[patch-panel-features] already applied")
    sys.exit(0)
...
idx = src.find(anchor)
if idx < 0:
    raise SystemExit("[patch-panel-features] App() not found")
...
if stat_close in src:
    ...
    if sec_close < 0:
        raise SystemExit("[patch-panel-features] cannot locate insert position")
    src = src[: sec_close + len("</section>")] + "\n\n        <FeaturesPanel />\n" + src[sec_close + len("</section>"):]

p.write_text(src)
print("[patch-panel-features] applied")
PY
