...
if "const stopLocation = (clientID: string, location: LocationState)" in t:
    print("[patch-panel-stop] already applied"); raise SystemExit(0)
    raise SystemExit(0)
...
if restart_fn not in t:
    print("[patch-panel-stop] restart function block not found"); raise SystemExit(0)
...
if restart_button not in t:
    print("[patch-panel-stop] skip (ui-v3 already has stop)"); raise SystemExit(0)
    raise SystemExit(0)
t = t.replace(restart_button, restart_button + "\n" + stop_button, 1)

p.write_text(t)
print("[patch-panel-stop] applied"); raise SystemExit(0)
PY
