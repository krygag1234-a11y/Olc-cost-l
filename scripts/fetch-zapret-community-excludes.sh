...
install -d "$DEST"
curl -fsSL --max-time 30 --retry 2 --retry-delay 3 -o "$DEST/flowseal-list-exclude.txt" "$BASE/list-exclude.txt"
curl -fsSL --max-time 30 --retry 2 --retry-delay 3 -o "$DEST/flowseal-ipset-exclude.txt" "$BASE/ipset-exclude.txt"
